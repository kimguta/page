...
function initSlick(target, options) {
	var ControlBtn = target.parents('section').find('.control .ps-btn');
	var PlayBtn  = target.parents('section').find('.control .play');
	var PauseBtn  = target.parents('section').find('.control .pause');
...
ObjDoc.on({
	'mouseover focusin': function() { 
		...
		$('.bg_pc').css('height',highestBox + 60).show();
